...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const { data: notifData } = useQuery<{ results: { IsRead: boolean }[] }>({
    ...
    queryFn: () =>
      axiosInstance
        .get("/api/notifications/?unread=true&page_size=50", { headers })
        .then((r) => r.data),
    ...
    <div className="flex h-screen overflow-hidden bg-surface-50">
      ...
      <aside className="flex w-[var(--sidebar-width)] shrink-0 flex-col border-r border-surface-200 bg-white">
        <div className="flex h-14 items-center border-b border-surface-100 px-4">
          <Link
            href="/dashboard"
            className="text-base font-bold text-brand-700"
          >
            SusDevOS
          </Link>
        </div>
        ...
        <div className="border-t border-surface-100 p-3">
          ...
          <div className="truncate px-1 text-xs text-surface-400">
            {user?.email}
          </div>
          <button
            onClick={() => {
