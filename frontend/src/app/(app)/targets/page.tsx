  ...
  return (
    <div className="space-y-1 min-w-[180px]">
      <div className="flex justify-between text-xs text-surface-500">
        ...
        {a !== null && (
          <span className={onTrack ? "text-brand-600" : "text-red-600"}>
            Actual: {fmt(actual)} t
          </span>
        )}
      </div>
      {b && (
        <div className="h-1.5 bg-surface-100 rounded-full overflow-hidden">
          <div
            ...
            style={{ width: `${Math.min(100, (t / b) * 100)}%` }}
          />
          {a !== null && (
            <div
...
function MilestonesRow({
  ...
  const { data, isLoading } = useQuery<Milestone[]>({
    ...
    queryFn: () =>
      axiosInstance
        .get(`/api/targets/${target.TargetId}/milestones/`, { headers })
        .then((r) => r.data),
  });
  ...
  if (isLoading)
    return (
      <tr>
        <td colSpan={6} className="px-4 py-3 text-xs text-surface-400">
          Loading milestones…
        </td>
      </tr>
    );
  ...
  if (milestones.length === 0)
    return (
      <tr>
        <td colSpan={6} className="px-4 py-3 text-xs text-surface-400 italic">
          No milestones. The nightly task links verified inventories to
  ...
  return (
    <>
      ...
      {milestones.map((m) => (
        <tr
          key={m.MilestoneId}
          className="bg-brand-50/50 border-t border-brand-100"
        >
          <td className="pl-10 pr-4 py-2.5 text-sm text-surface-700">
            {m.MilestoneYear}
          </td>
          ...
          <td className="px-4 py-2.5 text-sm tabular-nums text-surface-600">
            {fmt(m.TargetEmissionsTonnes)}
          </td>
          <td className="px-4 py-2.5 text-sm tabular-nums text-surface-600">
            {fmt(m.ActualEmissionsTonnes)}
          </td>
      ...
      axiosInstance.post(
        "/api/targets/",
        {
          ...
          ReductionPercent: form.ReductionPercent
            ? Number(form.ReductionPercent)
            : null,
          BaselineEmissionsTonnes: form.BaselineEmissionsTonnes
        ...
        >
          <div>
            ...
            <input
              className="input"
              required
              placeholder="e.g. 50% Scope 1+2 reduction by 2030"
          ...
          <div className="grid grid-cols-3 gap-3">
            <div>
              ...
              <input
                ...
                onChange={(e) => set("BaselineYear", e.target.value)}
              />
            </div>
            <div>
            ...
            <div>
              ...
              <input
                ...
                step={0.1}
                placeholder="50"
                value={form.ReductionPercent}
                onChange={(e) => set("ReductionPercent", e.target.value)}
              />
            </div>
          </div>
          <div>
            ...
            <input
              ...
              value={form.BaselineEmissionsTonnes}
              onChange={(e) => set("BaselineEmissionsTonnes", e.target.value)}
            />
            <p className="mt-1 text-xs text-surface-400">
              Used for milestone progress bars. Populated from verified
...
export default function TargetsPage() {
  ...
  const headers: Record<string, string> = activeEntityId
  ...
  const { data, isLoading } = useQuery<{ results: Target[] }>({
    ...
    queryFn: () =>
      axiosInstance.get("/api/targets/", { headers }).then((r) => r.data),
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="page-subtitle">
            Project goals and annual milestone tracking.
          </p>
        </div>
        <button
          ...
          <table className="table-auto w-full">
            ...
            <tbody>
              {targets.map((t) => {
                ...
                const vl =
                  VALIDATION_LABELS[t.ValidationStatus] ?? VALIDATION_LABELS[1];
                return (
                  <>
                    <tr
                      key={t.TargetId}
                    ...
                    >
                      ...
                      <td>
                        <span className={`badge ${vl.badge} text-xs`}>
                          {vl.label}
                        </span>
                      </td>
                    </tr>
                    {isOpen && <MilestonesRow target={t} headers={headers} />}
                  </>
                );
              })}
            </tbody>
