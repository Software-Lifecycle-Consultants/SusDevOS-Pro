...
export function DetailView(props: DetailViewProps) {
  ...
  const { data: row, isLoading, refetch } = useQuery<Row>({
    queryKey: [queryKey, id, activeEntityId],
    queryFn: () => axiosInstance.get(`${resourcePath}${id}/`, { headers }).then((r) => r.data),
  ...
  useEffect(() => {
    if (row && editing) {
      ...
      for (const f of fields) if (f.editable !== false) init[f.key] = str(row[f.key]);
      setForm(init);
    }
  }, [row, editing, fields]);
  ...
  const save = useMutation({
    mutationFn: (payload: Record<string, unknown>) =>
      axiosInstance.patch(`${resourcePath}${id}/`, payload, { headers }).then((r) => r.data),
    onSuccess: () => {
    ...
    onError: (err: unknown) => {
      const d = (err as { response?: { data?: { detail?: string; errors?: Record<string, string[]> } } })?.response?.data;
      setError(d?.errors ? Object.values(d.errors)[0]?.[0] ?? "Validation error." : d?.detail ?? "Save failed.");
    },
  });

  const del = useMutation({
    mutationFn: () => axiosInstance.delete(`${resourcePath}${id}/`, { headers }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [queryKey] });
  ...
  const action = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: [queryKey] }); refetch(); },
    onError: (err: unknown) => {
  ...
  if (!activeEntityId) return <p className="p-6 text-sm text-surface-500">Select an entity first.</p>;
  if (isLoading) return <p className="p-6 text-sm text-surface-500">Loading…</p>;
  ...
  return (
    <div className="space-y-4">
      <Link href={listHref} className="inline-flex items-center gap-1 text-sm text-surface-500 hover:text-surface-700">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          {props.subtitle && <p className="page-subtitle">{props.subtitle(row)}</p>}
        </div>
        <div className="flex gap-2">
          {props.actions?.filter((a) => !a.show || a.show(row)).map((a) => (
            <button
              key={a.label}
              className={a.variant === "danger" ? "btn-secondary text-red-600" : "btn-secondary"}
          ...
          {!editing && canEdit && <button className="btn-primary" onClick={() => setEditing(true)}>Edit</button>}
          {!editing && canDelete && (
            <button className="btn-secondary text-red-600" disabled={del.isPending}
              onClick={() => { if (window.confirm("Delete this record?")) del.mutate(); }}>
              Delete
            </button>
          )}
        </div>
      ...
      <div className="card p-6">
        {editing ? (
          <form onSubmit={submit} className="space-y-4">
            {fields.filter((f) => f.editable !== false).map((f) => (
              <div key={f.key}>
                ...
                {f.type === "textarea" ? (
                  <textarea className="input resize-y min-h-[72px]" value={form[f.key] ?? ""}
                  ...
                  <select className="input" value={form[f.key] ?? "false"}
                    onChange={(e) => setForm((s) => ({ ...s, [f.key]: e.target.value }))}>
                  ...
                  <input className="input" type={f.type === "number" ? "number" : f.type === "date" ? "date" : "text"}
                    step={f.type === "number" ? "any" : undefined} value={form[f.key] ?? ""}
                    onChange={(e) => setForm((s) => ({ ...s, [f.key]: e.target.value }))} />
                )}
                {f.help && <p className="mt-1 text-xs text-surface-400">{f.help}</p>}
              </div>
            ))}
            <div className="flex gap-2 pt-2">
              <button type="button" className="btn-secondary" onClick={() => { setEditing(false); setError(null); }}>Cancel</button>
          ...
          <dl className="grid grid-cols-1 gap-x-8 gap-y-3 sm:grid-cols-2">
            {fields.map((f) => (
              <div key={f.key}>
                <dt className="text-xs font-medium uppercase tracking-wider text-surface-400">{f.label}</dt>
                <dd className="mt-0.5 text-sm text-surface-800">
                  {f.render ? f.render(row[f.key], row) : (str(row[f.key]) || "—")}
                </dd>
              </div>
            ))}
          </dl>
