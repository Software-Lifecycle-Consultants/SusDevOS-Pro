# SusDevOS — C4 Architecture Model

The C4 model gives four levels of zoom into the SusDevOS platform: from the top-level view
of users and external dependencies (Level 1) down to how a specific calculation executes in
code (Level 4). Each level is a drill-down of the level above it.

These diagrams complement the UML and BPMN sets already in this directory. UML shows domain
models and request sequences; BPMN shows process swimlanes; C4 shows structural decomposition
across container and component boundaries.

---

## Level 1 — System Context

Who uses SusDevOS, and which external systems does it depend on?
...
What deployable units make up the platform, and how do they communicate?

```mermaid
C4Container
    title Container Diagram — SusDevOS Platform (Contabo VPS · Ubuntu 24.04)
    ...
    System_Boundary(platform, "SusDevOS Platform") {
        ...
        Container(spa, "Web Application", "Next.js 14 · TypeScript · shadcn/ui · Tailwind", "App Router SPA. TanStack Query for server state. Zustand auth store (JWT in memory only). orval-generated TypeScript REST client from OpenAPI spec.")
    ...
    System_Ext(ext, "External APIs", "Climatiq · Verra · Gold Standard · ECB · OER · GBIF · IUCN · Companies House · SMTP/SES")
    ...
    Rel(api, broker, "Enqueues background tasks (e.g. generate_report.delay)", "Redis protocol")
...
## Level 3a — Components: API Request Pipeline

How does a request flow through the Django API from entry to ViewSet action?

Every authenticated API request passes through this pipeline in order. A failure at any stage
returns an error response immediately and the request never reaches the next stage.
...
C4Component
    ...
    Container_Boundary(api, "Django REST API") {
        ...
        Component(gate, "FeatureGateMixin", "DRF ViewSet mixin", "Short-circuited when FEATURE_GATES_ENABLED=False (current default). When enabled: checks PlanFeatures table and returns HTTP 402 with {code: feature_gated, upgrade_url: /pricing} if not entitled.")
        Component(mixin, "TenantViewSetMixin", "DRF ViewSet mixin", "Overrides get_queryset() to always append .filter(EntityId=request.entity_id, Status__lt=4). EntityId from request body is never trusted — always overwritten with request.entity_id.")
        ...
        Component(spec, "OpenAPI Schema endpoint", "drf-spectacular", "Generates OpenAPI 3 spec at /api/schema/. Frontend build runs orval against this spec to emit typed TypeScript client and TanStack Query hooks in src/lib/api/.")
    }
    ...
    Rel(mw, auth, "Passes request with entity_id resolved on request object")
    Rel(auth, pg, "Checks RevokedTokens by Jti UUID", "SQL")
    Rel(auth, perm, "Passes authenticated (user, token) tuple")
...
## Level 3b — Components: Django Domain Apps

The twelve Django apps, their load order, and what each owns.

Apps early in the load order are depended on by apps later in the order; no reverse imports.

```mermaid
C4Component
    title Component Diagram — Django API: Domain Apps (INSTALLED_APPS load order)
    ...
    Container_Boundary(api, "Django REST API — 12 Domain Apps") {
        ...
        Component(blog, "blog", "Django App (10th)", "BlogPosts, BlogCategories. Draft/pending/published state machine. Rich-text content management for the marketing site and help content.")
        ...
        Component(billing, "billing", "Django App (12th)", "Plans, PlanFeatures, Subscriptions, UsageTracking. Plan limit resolution (MaxEntities, MaxUsersPerEntity, MaxReportingYears, MaxApiCallsPerDay). Limit enforcement callers are present but not yet wired.")
    }
```

App dependency direction: `shared` ← `entities` ← `users` ← `projects` ← `land` ← `ecosystem`
← `emissions` ← `restorations` ← `notifications` ← `blog` ← `reports` ← `billing`

---

## Level 3c — Components: Celery Async Worker Layer

How are background tasks structured, scheduled, and routed across worker pools?
...
C4Component
    ...
    Container_Boundary(workers, "Celery Worker Layer") {
        Component(beat, "celery beat", "Celery Beat · DatabaseScheduler", "Dispatches 11 scheduled tasks. beat_init validator (see F6) ensures every beat_schedule entry is registered before the scheduler starts.")
        ...
        Component(w_emissions, "tasks.emissions", "Default queue", "recompute_stale_inventory_totals (01:00 daily): re-aggregates scope totals into GHGInventories. link_milestone_actuals (01:30 daily): fills TargetMilestones.ActualEmissionsTonnes from confirmed records.")
        Component(w_climatiq, "tasks.integrations.climatiq", "Integrations queue", "sync_climatiq_emission_factors (02:00 UTC every Sunday). Refreshes EmissionFactorLibrary from the Climatiq API. Isolated to the integrations queue because network calls are slow.")
        Component(w_registry, "tasks.integrations.verra_gs", "Integrations queue", "sync_verra_registry (03:00 daily). sync_gold_standard_registry (03:30 daily). Validates active credit serials against registry APIs, updates validation status.")
        Component(w_fx, "tasks.integrations.fx", "Integrations queue", "sync_ecb_fx_rates (17:00 daily): ECB reference rates published ~16:00 CET. sync_oer_fx_rates: Open Exchange Rates fallback (on demand). Writes to Currencies exchange rate rows.")
        Component(w_reports, "tasks.reports", "Reports queue · max-tasks-per-child=10", "generate_report (on demand): renders PDF/CSV from ReportJob config, uploads to S3, updates job status and pre-signed URL. purge_expired_reports (04:00 daily): removes expired ReportJobs and S3 objects. max-tasks-per-child=10 recycles worker memory after PDF rendering.")
    }
...
## Level 4 — Code: Server-Side GHG Calculation Pipeline

The calculation engine is the most security-critical code path in the platform. The client
**never** computes or submits `EmissionsAmount`; it submits raw activity data and receives
back a fully calculated, server-authoritative record.
...
**Key invariants enforced at this level:**

| Invariant | Where enforced |
|-----------|---------------|
| Client never computes emissions | `EmissionsData.save()` overwrites all result columns |
| EntityId never from request body | Serializer replaces it with `request.entity_id` |
| Biogenic CO2 excluded from GWP total | Separate `BiogenicCO2AmountTonnes` column |
| Scope 2 requires both methods | Both `LocationBased` and `MarketBased` columns always computed |
| Verified records are immutable | `VerificationStatus >= 3` → HTTP 403 on PATCH/DELETE |
